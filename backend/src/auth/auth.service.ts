...
export class AuthService {
    ...
    try {
      const response = await fetch('https://api.intra.42.fr/oauth/token', {
        ...
        body: JSON.stringify({
          ...
          client_secret:
            await this.credentialService.getClientSecretByProvider('42'),
          code: code,
  ...
  private async saveSession(req: Request, account: Account) {
    req.session.user = {
      id: account.id,
      roles: await this.roleRepository.findRolesByAccountId(account.id),
    };
  ...
  async redirectTo42(res: Response) {
    ...
    const redirectUri = encodeURIComponent(
      process.env.VITE_API_URL + '/auth/42/callback',
    );
