...
import { ApiRes } from "@/services/core.service";
...
interface AccountFieldEditorProps {
  fieldName: string;
  fieldValue: string;
  validationSchema: any;
  request: (value: string) => Promise<ApiRes>;
  label: string;
  placeholder: string;
}

export const AccountFieldEditor = ({ fieldName, fieldValue, validationSchema, request, label, placeholder }: AccountFieldEditorProps) => {
  ...
  const onSubmit = async (values: typeof validationSchema) => {
    ...
    const response = await request(fieldValue);
    if (response.success) {
      setSuccess(true);
      setEdit(false);
      setErrorMsg("");
  ...
  return (
    <div>
      <div className="flex items-center gap-2">
        ...
        {success && <span className="text-sm font-medium text-green-500">{t("core.messages.save")}</span>}
      </div>
      {errorMsg && (
